...
# 2. Train the Classifier (SVM with RBF kernel for curved decision boundaries)
clf = svm.SVC(kernel='rbf', C=2.0, gamma=0.005)
clf.fit(X, y)
...
# Dashed decision boundaries (Cyan/Blue themed)
...
ax.set_title('Martian Classification: Height vs. Weight)', fontsize=15, color='#f8fafc', pad=15)
...
legend_elements = [
    ...
    Line2D([0], [0], color='#38bdf8', linestyle='dashed', lw=2, label='Decision Boundary')
]
...
plt.savefig('martian_classification.png', dpi=300, facecolor=fig.get_facecolor(), edgecolor='none')
